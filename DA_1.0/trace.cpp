...
{
	printf("%6s  %10s   %-20s %4s %20s %10s\n", "Index", "L_time", "Datablk_set", "G_value", "reponse_times", "Times_Straggler");
...
//			printf(" %2lld ", ReadTrace[i].datablks[j] % ec_node);
		...
		for(int j = 0; j < ReadTrace[i].reponse_times.size(); j++)
		{
			printf(" %lld ", ReadTrace[i].reponse_times[j]);
		}
